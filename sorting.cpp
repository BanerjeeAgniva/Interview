...
// bubble sort ---> pair-by-pair comparison for the entire length for a total of
// n-iterations insertion sort ---> for an array of length n, you first sort out
// till length 1, then till length 2, .... till length n
//-----
...
void mergesort(vector<int> &arr, int left, int right) {
  if (left >= right)
    return; // no partitioning possible when one element left
  int mid = left + (right - left) / 2;
  mergesort(arr, left,
            mid); //     mergesort(arr,left,mid-1);   ---> This leads to
  mergesort(arr, mid + 1, right); //      mergesort(arr,mid,right); SEG FAULT
  merge(arr, left, mid, right);
}
void mergesort(vector<int> &arr) { mergesort(arr, 0, arr.size() - 1); }
//----------
int partition(vector<int> &arr, int left,
              int right) // place pivot element in correct position
{
  int pivot = arr[left];
  int i = left + 1;
  int j = right;
  while (true) {
    while (arr[i] < pivot)
      i++; // we need to send greater elements than pivot to the right side
    while (arr[j] > pivot)
      j--; // we need to keep smaller elements than pivot to the left side
    if (j < i)
      break;
    swap(arr[i], arr[j]);
  }
  swap(
      arr[j],
      arr[left]); // put the pivot element = arr[left] in its correct position j
  return j;
}
...
void quicksort(vector<int> &arr) { qs(arr, 0, arr.size() - 1); }
...
3 modifications to merge sort code
