...
void add_dnode(struct dnode* root, int pos, int data) // add dnode after "pos" index 
{
...
struct dnode* deletednode(struct dnode* root, int x) {  //delete dnode at "x" position
...
void printDLL(struct dnode* root)
{
    ...
    while(temp){
        cout<<temp->val<<" ";
        if(temp->prev) cout<<temp->prev->val<<" ";
        else cout<<"NO PREVIOUS ";
        if(temp->next) cout<<temp->next->val<<" ";
        else cout<<"NO NEXT ";
        temp=temp->next;
        cout<<"\n";
    }
...
>>>>>>> Stashed changes:LinkedList.cpp
